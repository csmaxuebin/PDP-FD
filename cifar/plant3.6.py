...
# 加载数据
with open('plant_data/dp10-fedkd34-85.4925.txt', 'r') as f1:
    data1 = f1.readlines()  # 加载第一个文件的数据

with open('plant_data/pj-dp10-fedkd34-84.31.txt', 'r') as f2:
    data2 = f2.readlines()  # 加载第二个文件的数据

with open('plant_data/dddj-fedkd34-dp10-82.05.txt', 'r') as f3:
...
# 绘制每个文件的准确率数据
plt.plot(rounds, acc1, color='#73BCD5', linestyle='-', label='PDP-FD (ε=10)')
plt.plot(rounds, acc2, color='#FFD06E', linestyle='-', label=' DP-FD (ε=10,Fixed)')
plt.plot(rounds, acc3, color='#ef8a46', linestyle='-', label=' DP-FD (ε=10,Uniform)')
