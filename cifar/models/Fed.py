...
def DiffPrivFedAvg(w):
    ...
    for i in range(len(w)):
        ...
        #probability of selecting the original weights 选择原始权重的概率，保留原始参数
        p = 0.8
        ...
        if(a<=p):
            ...
            for keys in w_temp.keys():
                ...
                # set very small elements to zero  设置小的元素为0
                
                alpha[np.abs(alpha) < epsilon] = 0
                
                alpha = alpha + 0.000005
                
                ele_square = np.power(alpha,2)    #每个元素进行平方运算，得到一个新的数组ele_square，其中每个元素都是原数组对应元素的平方
                
                ele_sum = np.sum(ele_square)      # 所有元素进行求和，得到其平方和
                
                # Divide sum of squares value by size of tensor to get standard deviation

                ele_val = ele_sum.item()/alpha.size     # 计算标准差

                # Generate gaussian noise of same shape as that of model weights

                w_temp[keys] = np.random.normal(0,ele_val,np.shape(w_temp[keys])) #生成一个与模型权重w_temp[keys]相同形状的高斯分布噪声，其中0表示高斯分布的均值，ele_val是标准差
                
                w_temp[keys] = torch.from_numpy(w_temp[keys])                # 将生成的噪声转换成Tensor类型
                
                w_temp[keys] = w_temp[keys].type(torch.cuda.FloatTensor)     # 转换为CUDA浮点类型
