...
import torch
from models.Nets import MLP, CNNMnist, CNNCifar, mycnn, ResNet, MobileNet,Net, NewNet, customResNet, customMobileNet162, customMobileNet138, customMobileNet150
...
def Load_Model(args):
    ...
    if args.model == 'cnn' and args.dataset == 'cifar':
        ...
        net_glob = mycnn(args=args).to(args.device)
...
def Load_Model1(args):
    ...
    if args.model == 'cnn' and args.dataset == 'cifar':
        ...
        for x in img_size:
            len_in *= x
        Ensemble_model = MLP(dim_in=len_in, dim_hidden=64, dim_out=args.num_classes).to(args.device)
    elif args.model == 'MobileNet':
        Ensemble_model = MobileNet(args=args).to(args.device)
    elif args.model == 'mycnn':
        Ensemble_model = mycnn(args=args).to(args.device)
    elif args.model == 'ResNet':
